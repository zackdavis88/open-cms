...
const getLayoutsValidation: GetLayoutsValidation = async ({ query, project }) => {
  ...
  const layoutCount = await Layout.count(dbQuery);
  const paginationData = validatePagination(query, layoutCount);
...
export default getLayoutsValidation;
